...
float paraboloid_intersection(t_paraboloid parab, float3 ray_start, float3 ray_dir, float *t1, float *t2)
{
    float3 parab_dir;
    float a;
    float b;
											...
											int cs_nmb, __global t_material *material_buf)
{
	...
	if (bounce_cnt == 0 || material_buf[i].reflection > 0.0f)
		result = paraboloid_intersection(parab, ray, ray_arr[i], &t1, &t2);
	else
		return ;
