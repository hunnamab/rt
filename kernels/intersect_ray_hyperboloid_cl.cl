...
float hyperboloid_intersection(t_hyperboloid hyper, float3 ray_start, float3 ray_dir, float *t1, float *t2)
{
 	float k1;
    float k2;
    float k3;
										...
										int cs_nmb, __global t_material *material_buf)
{
	...
	if (bounce_cnt == 0 || material_buf[i].reflection > 0.0)
		result = hyperboloid_intersection(hyper, ray, ray_arr[i], &t1, &t2);
	else
		return ;
