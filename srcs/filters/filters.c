...
void    sepia_filter(t_filter_data data)
{
    ...
    w = WID;
    h = HEI;
    clSetKernelArg(data.kernels[SEPIA], 0, sizeof(cl_mem), &data.pixels);
	clSetKernelArg(data.kernels[SEPIA], 1, sizeof(cl_int), &w);
	clSetKernelArg(data.kernels[SEPIA], 2, sizeof(cl_int), &h);
    clGetKernelWorkGroupInfo(data.kernels[SEPIA], data.device_id, CL_KERNEL_WORK_GROUP_SIZE, sizeof(local), &local, NULL);
    clEnqueueNDRangeKernel(data.commands, data.kernels[SEPIA], 1, NULL, &global, &local, 0, NULL, NULL);
