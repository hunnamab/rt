...
/*   Updated: 2021/02/04 21:16:28 by ldeirdre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		intersect_ray_triangle(t_scene *scene, int index)
{
	...
	clSetKernelArg(scene->cl_data.kernels[4], 6, sizeof(cl_int), (void*)&scene->bounce_cnt);
	clSetKernelArg(scene->cl_data.kernels[4], 7, sizeof(cl_mem), &cs);
	clSetKernelArg(scene->cl_data.kernels[4], 8, sizeof(cl_int), (void*)&scene->objs[index]->cs_nmb);
	clSetKernelArg(scene->cl_data.kernels[4], 9, sizeof(cl_mem), &scene->cl_data.scene.material_buf);
