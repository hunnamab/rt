...
void        intersect_ray_paraboloid(t_scene *scene, int index)
{
	...
	clSetKernelArg(scene->cl_data.kernels[12], 6, sizeof(cl_float), (void*)&scene->objs[index]->reflection);
	clSetKernelArg(scene->cl_data.kernels[12], 7, sizeof(cl_int), (void*)&scene->bounce_cnt);
	clSetKernelArg(scene->cl_data.kernels[12], 8, sizeof(cl_mem), &cs);
	clSetKernelArg(scene->cl_data.kernels[12], 9, sizeof(cl_int), (void*)&scene->objs[index]->cs_nmb);
